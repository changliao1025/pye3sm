...
from eslib.visual.surface.surface_plot_data_monthly_maya import surface_plot_data_monthly_maya
...
def elm_surface_plot_variable_halfdegree_domain(sFilename_configuration_in,\
    ...
    #region mesh
        ...
        aData = aVariable2[ lat_min:lat_max, lon_min:lon_max  ]
       
        print(np.min(aGrid_x), np.max(aGrid_x), np.min(aGrid_y), np.max(aGrid_y))
        surface_plot_data_monthly_maya(aGrid_x, \
            aGrid_y,\
            np.array(aData),\
                                      sFilename_out,\
                                      iReverse_z_in = 1, \
