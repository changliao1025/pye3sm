    ...
    for iRow in range(iRow_start, iRow_end+1, 1):
       ...
       for iColumn  in range(1, ncolumn+1, 1):
            ...
            if np.isnan(aWtd).all():
                ...
                #if(  np.min(aWtd) <= dWtd and dWtd <= np.ma(aWtd)  ):
                #    aQC[iRow, iColumn] = 1
                ...
                #    aAnisotropy_optimal[iRow, iColumn] =np.powe(10.0, dummy[0]   )
                #else:
                #    if( np.min(aWtd) > dWtd ):
                #        aAnisotropy_optimal[iRow, iColumn] =nppower(10.0,\
                #                aAnisotropy_sort[nCase -1]   )
                #        aQC[iRow, iColumn] = 2
                #    else:
                #        aAnisotropy_optimal[iRow, iColumn] = nppower(10.0,aAnisotropy_sort[0] )
                #        aQC[iRow, iColumn] = 3
