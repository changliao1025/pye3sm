...
def elm_stat_forcing_data_2d(oE3SM_in, oCase_in, sVariable_forcing_in, iFlag_scientific_notation_colorbar_in =None,   \
                                          dData_max_in = None,\
                                          dData_min_in = None,
                                         sUnit_in=None,\
                                          sTitle_in =None):
    ...
    index_date =0 
    start_row =  int( (90-(dLat_max+0.5*dResolution_y)) / dResoultion_forcing )
    start_column = int( (dLon_min-0/5*dResolution_x-(-180)) / dResoultion_forcing )
    for iYear in range(iYear_start, iYear_end + 1, 1):
        ...
        for iMonth in range(1, 12 + 1, 1):
            ...
            for sFilename in glob.glob(sRegex):
                ...
                total_prec = np.sum(aData,axis=0)
                ...
                for i in range(nrow_extract):
                    for j in range(ncolumn_extract):
                        ...
                        if iMask >=1:
                            ...
                            aData_out_extract[index_date,i,j] = total_prec[iIndex, jIndex]
    ...
    sFilename_out=sWorkspace_analysis_case_region + slash \
                         + sVariable_forcing_in + '_map_total_GWSP' + '.png'    
