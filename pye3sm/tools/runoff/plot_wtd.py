...
def plot_wtd():
    ...
    sFilename_runoff = '/qfs/people/liao313/data/e3sm/amazon/elm/' + 'wtd_extract' + sExtension_tiff
    if os.path.exists(sFilename_runoff):
        ...
        aImage_in[np.where(aImage_in > 12)] = 12
        #aImage_in=aImage_in/30

        cmap = cm.get_cmap('rainbow')
        # 
        demplot = ax_dem.imshow(aImage_in, origin='upper', extent=aImage_extent,cmap = cmap,transform=crs) 
        # change all spines
        ...
        cb.ax.set_ylabel('Unit: m', rotation=270)
        cb.ax.tick_params(labelsize=6) 

        sFilename_out = '/compyfs/liao313/04model/e3sm/amazon/analysis/gp/wtd.png'
