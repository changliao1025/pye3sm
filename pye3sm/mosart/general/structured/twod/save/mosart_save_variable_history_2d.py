...
def mosart_save_variable_history_2d(oE3SM_in, oCase_in):
    ...
    sYear = "{:04d}".format(iYear_end+1) #str(iYear).zfill(4)
    
        #for iMonth in range(iMonth_start, iMonth_end + 1):
        #    sMonth = str(iMonth).zfill(2)
    
    sDummy = '.mosart.r.' + sYear + '-' + '01-01-00000' + sExtension_netcdf
    ...
    for sKey, aValue in aDatasets.variables.items():
        if sKey.lower() == sVariable.lower() :
            ...
            gdal_write_envi_file(sFilename_envi, aData_ul,\
            ...
            gdal_write_geotiff_file(sFilename_tiff, aData_ul,\
                float(pHeaderParameters['pixelSize']),\
                 float(pHeaderParameters['ULlon']),\
                      float(pHeaderParameters['ULlat']),\
                          -9999.0, pSpatial)
